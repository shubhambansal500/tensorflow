...
def compare2face(img1, videoImg):
    face1 = getFace(img1)
    face2 = getFace(videoImg)
    if face1 and face2:
...
def capture_image():
    cam = VideoCapture(0)
    global videoimage
    s, videoimage = cam.read()
    if s:
        namedWindow("Taking Image...")
        imshow("Taking Image...", videoimage)
        waitKey(0)
        imwrite("./tmp/shubham.jpg", videoimage)
...
videoImg = videoimage
distance = compare2face(img1, videoImg)
